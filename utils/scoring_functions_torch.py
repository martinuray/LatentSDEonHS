...
class Evaluator:
    ...
    def auc(self, labels: torch.Tensor, scores: torch.Tensor) -> Tuple[
        float, Dict[str, Any]]:
        """
    ...
    def f1_score(self, labels: torch.Tensor, scores: torch.Tensor,
                 pos_label: int = 1) -> Tuple[float, Dict[str, Any]]:
        ...
        return f1_score(labels.numpy(), scores.numpy(),
                        pos_label=pos_label).item(), {}

    def best_fbeta_score(self, labels: torch.Tensor, scores: torch.Tensor,
                         beta: float) -> Tuple[float, Dict[str, Any]]:
        r"""
        ...
        """
        precision, recall, thresholds = precision_recall_curve(labels.numpy(),
                                                               scores.numpy())

        f_score = np.nan_to_num((1 + beta ** 2) * precision * recall / (
                    beta ** 2 * precision + recall), nan=0)
        best_index = np.argmax(f_score)

        return f_score[best_index].item(), dict(
            threshold=thresholds[best_index].item())

    def best_f1_score(self, labels: torch.Tensor, scores: torch.Tensor) -> \
    ...
    def auprc(self, labels: torch.Tensor, scores: torch.Tensor,
              integration: str = 'trapezoid') -> Tuple[float, Dict[str, Any]]:
        r"""
    ...
    def average_precision(self, labels: torch.Tensor, scores: torch.Tensor) -> \
    Tuple[float, Dict[str, Any]]:
        r"""
    ...
    def ts_auprc(self, labels: torch.Tensor, scores: torch.Tensor,
                 integration='trapezoid',
                 weighted_precision: bool = True) -> Tuple[
        float, Dict[str, Any]]:
        """
        ...
        precision = torch.empty(thresholds.shape[0] + 1, dtype=torch.float,
                                device=thresholds.device)
        recall = torch.empty(thresholds.shape[0] + 1, dtype=torch.float,
                             device=thresholds.device)
    ...
    def ts_average_precision(self, labels: torch.Tensor, scores: torch.Tensor,
        ...
        return self.ts_auprc(labels, scores, integration='riemann',
                             weighted_precision=weighted_precision)

    def ts_auprc_unweighted(self, labels: torch.Tensor, scores: torch.Tensor) -> \
    Tuple[float, Dict[str, Any]]:
        """
        Compute the area under the precision-recall curve using precision and recall for time series [Tatbul2018]_.
        ...
        :return: A tuple consisting of the AuPRC score and an empty dict.
        """
        return self.ts_auprc(labels, scores, integration='trapezoid',
                             weighted_precision=False)

    def __best_ts_fbeta_score(self, labels: torch.Tensor, scores: torch.Tensor,
                              beta: float,
    ...
    def best_ts_fbeta_score_classic(self, labels: torch.Tensor,
                                    scores: torch.Tensor, beta: float) -> Tuple[
        float, Dict[str, Any]]:
        r"""
        Compute the :math:`F_{\beta}` score using precision and recall for time series [Tatbul2018]_.
    ...
    def best_ts_f1_score(self, labels: torch.Tensor, scores: torch.Tensor) -> \
    Tuple[float, Dict[str, Any]]:
        r"""
    ...
    def best_ts_f1_score_classic(self, labels: torch.Tensor,
